"""Experiment orchestration for AirRaid DWN training with evaluations and checkpoints."""
...
from airraid.airraid_dwn import AirRaidDWNArgs, AirRaidDWNTrainer


class AirRaidDWNExperiment:
    """Experiment wrapper for AirRaid DWN training with periodic evaluation and checkpointing.

    Mirrors DWNExperiment (bidding_gridworld/dwn_experiment.py) but uses
    step-based callbacks with the AirRaidDWNTrainer.
    """
    ...
    def save_config(self, args: AirRaidDWNArgs):
        config_file = self.config_dir / "training_config.json"
        with open(config_file, "w") as f:
            json.dump(vars(args), f, indent=2, default=str)
        print(f"Config saved to {config_file}")

    def save_checkpoint(self, trainer: AirRaidDWNTrainer, global_step: int):
    ...
    def evaluate_policy(
        ...
        env_config = AirRaidConfig(
            ...
            enemy_missile_danger_penalty=args.enemy_missile_danger_penalty,
            enemy_missile_danger_y_threshold=args.enemy_missile_danger_y_threshold,
            enemy_missile_danger_x_radius=args.enemy_missile_danger_x_radius,
            enemy_missile_danger_y_radius=args.enemy_missile_danger_y_radius,
            enemy_missile_near_hit_penalty=args.enemy_missile_near_hit_penalty,
            enemy_missile_near_hit_y_margin=args.enemy_missile_near_hit_y_margin,
            enemy_missile_near_hit_x_radius=args.enemy_missile_near_hit_x_radius,
            enemy_disappear_penalty=args.enemy_disappear_penalty,
    ...
    def run(self, args: AirRaidDWNArgs):
        """Run the full training experiment."""
        print(f"\n{'='*80}")
        print("AIRRAID DWN TRAINING")
        print(f"{'='*80}\n")
        ...
        def on_step(trainer: AirRaidDWNTrainer, global_step: int):
            if (
        ...
        def on_training_end(trainer: AirRaidDWNTrainer, global_step: int):
            self.save_checkpoint(trainer, global_step)
            print("\n" + "=" * 80)
