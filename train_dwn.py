#!/usr/bin/env python3
"""
DWN Training Script for Bidding Gridworld

Trains a Deep W-Learning policy for multi-agent bidding with shared Q/W networks.
One shared Q-network proposes directions per agent; one shared W-network selects
the winner. The winning agent bids 1, all others bid 0.
...
def main():
    ...
    # Experiment
    EXPERIMENT_NAME = "dwn_moving_targets_exp1"
    SEED = 1
    ...
    NUM_VIDEO_EPISODES = 0        # Episodes to save as MP4s
    LOG_VIDEOS_TO_WANDB = False   # Upload MP4s to wandb
    EVAL_MAX_STEPS   = 2000       # Max steps per eval episode

    # ---- Environment --------------------------------------------------------

    GRID_SIZE = 30
    NUM_TARGETS = 6            # Number of bidding agents / objectives
    TARGET_REWARD = 50.0
    MAX_STEPS = 2000           # Max steps per episode

    DISTANCE_REWARD_SCALE = 0.6  # Reward for closing distance to own target

    TARGET_EXPIRY_STEPS = 200    # Steps before an unreached target expires (None = no expiry)
    ...
    TARGET_MOVE_INTERVAL = 5    # Steps between target moves
    ...
    BATCH_SIZE = 256
    LEARNING_STARTS = 100_000    # Steps before first Q update
    TRAIN_FREQUENCY = 2_560     # Update Q every N global steps; must be >= NUM_ENVS (global_step increments by NUM_ENVS each iteration, so any value < NUM_ENVS fires every step)
    W_TRAIN_DELAY = 1_000_000     # Additional steps after LEARNING_STARTS before W updates begin
    ...
    LOG_FREQUENCY = 25_600      # Print + wandb log every N steps (~100 global steps at NUM_ENVS=256)
