#!/usr/bin/env python3
"""
PPO Training Script for Moving Targets Bidding Gridworld

This script trains a shared PPO policy for multi-agent bidding with moving targets.
...
class PPOMovingTargetsExperiment:
    ...
    def __init__(
        ...
        if not experiment_name:
            experiment_name = "ppo_moving_targets"
    ...
    def save_config(self, args: Args):
    ...
    def save_checkpoint(self, trainer: PPOTrainer, iteration: int, global_step: int):
    ...
    def run(self, args: Args):
        ...
        print(f"\n{'='*80}")
        print("PPO TRAINING WITH MOVING TARGETS")
        print(f"{'='*80}\n")
...
def main():
    ...
    # Create PPO Args
    ppo_args = Args(
        exp_name=EXPERIMENT_NAME or "ppo_moving_targets",
        seed=SEED,
        track=TRACK,
