#!/usr/bin/env python3
"""
Action Window Comparison Experiment

Runs experiments in parallel: multi-agent PPO with all_pay mechanism,
varying action_window across [1, 2, 5, 10, 20].
...
BASE_LOG_DIR = "logs/gridworld_action_window_comparison"
...
def make_ppo_args(action_window: int, exp_name: str) -> Args:
...
def run_ppo_experiment(action_window: int, exp_name: str) -> None:
    ...
    args = make_ppo_args(action_window, exp_name)
...
def main():
    experiments = [
        (
            ...
            f"action_window_cmp_w{w}",
            functools.partial(run_ppo_experiment, w, f"action_window_cmp_w{w}"),
        )
