#!/usr/bin/env python3
"""
Bid Upper Bound Comparison Experiment

Runs 4 experiments in parallel: multi-agent PPO with all_pay mechanism,
varying bid_upper_bound across 0, 2, 4, 6.

bid_upper_bound=0 uses random winner selection (no bidding).
...
BASE_LOG_DIR = "logs/gridworld_bid_upper_bound_comparison"
...
# Maximum bid penalty (bid_penalty * bid_upper_bound) held constant across runs.
# bid_penalty is derived per-run as MAX_BID_PENALTY / bid_upper_bound.
# For bid_upper_bound=0 (random winner), bid_penalty is irrelevant (all bids are 0).
MAX_BID_PENALTY = 0.6
ACTION_WINDOW = 5
...
# bid_upper_bound values to sweep
BID_UPPER_BOUNDS = [0, 1, 2, 4]
...
def make_ppo_args(bid_upper_bound: int, exp_name: str) -> Args:
    """Build an Args instance for a multi-agent PPO run."""
    # Keep max bid penalty constant: bid_penalty * bid_upper_bound = MAX_BID_PENALTY.
    # bid_upper_bound=0 uses random winner selection; bid_penalty is irrelevant there.
    bid_penalty = MAX_BID_PENALTY / bid_upper_bound if bid_upper_bound > 0 else 0.0
    return Args(
        exp_name=exp_name,
        seed=SEED,
        track=TRACK,
...
def run_ppo_experiment(bid_upper_bound: int, exp_name: str) -> None:
    """Create and run a multi-agent PPO experiment."""
    args = make_ppo_args(bid_upper_bound, exp_name)
...
def main():
    experiments = [
        (
            f"Multi-agent PPO — all_pay — bid_upper_bound={bub}",
            f"bid_ub_cmp_bub{bub}",
            functools.partial(run_ppo_experiment, bub, f"bid_ub_cmp_bub{bub}"),
        )
        for bub in BID_UPPER_BOUNDS
    ]
