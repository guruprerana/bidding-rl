#!/usr/bin/env python3
"""
Plot peak performance bar chart for the action window comparison experiment.

For each action window value the peak mean 8 × avg_performance across all
eval checkpoints is selected.

Usage:
    python experiment_scripts/plots/plot_action_window_comparison.py
    python experiment_scripts/plots/plot_action_window_comparison.py --log-dir logs/gridworld_action_window_comparison
"""
...
LOG_DIR = "logs/gridworld_action_window_comparison"
NUM_AGENTS = 8
CONFIDENCE = 0.95

EXPERIMENTS = [
    ("action_window_cmp_w2",  "2"),
    ("action_window_cmp_w5",  "5"),
    ("action_window_cmp_w10", "10"),
    ("action_window_cmp_w20", "20"),
]
...
def main():
    ...
    output_path = args.output or os.path.join(log_dir, "gridworld_action_window_bar_chart.png")
    ...
    for exp_prefix, label in EXPERIMENTS:
        ...
        print(f"  action_window={label}: peak at step {steps[peak_idx]:,}, "
              f"8 × avg_performance = {peak_mean:.3f}")
    ...
    ax.bar(x, bar_heights, yerr=[err_lo, err_hi],
           capsize=5, width=0.6, color="mediumseagreen", alpha=0.8)
